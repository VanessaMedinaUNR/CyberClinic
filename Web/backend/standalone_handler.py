...
class StandaloneHandler:
    ...
    def start_auth_tunnel(self, bindsocket: socket.socket, authed_socket: socket.socket):
        logger.debug(f'loading {self.auth_cert}')
        logger.debug(f'loading {self.auth_key}')
        ...
        if response:
            ...
            match command:
                ...
                case "SEND_RESULTS":
                    ...
                    logger.debug(f'Saving results to {of}...')
                    recvd = 0
                    with open(f'{of}', 'wb') as f:
                        while recvd < result_size:
                            results = conn.recv(1024)
                            recvd += len(results)
                            f.write(results)

                    logger.debug(f"Received {os.path.getsize(of)} bytes for scan {scan_id} of report {report_id} from {fromaddr}")
                    conn.send(b'RESULT_RECEIVED')

                    result_path = json.dumps({ext: of})
                    ...
                    if os.path.exists(of) and os.path.getsize(of) == result_size:
                        ...
                        self._mark_scan_completed(scan_id, result_path)
                    else:
    ...
    def parse_auth_request(self, conn: ssl.SSLSocket, fromaddr, auth_string: str) -> tuple[bool, tuple[str, str]]:
        try:
            db = get_db()
                ...
                if success:
                    ...
                    if not valid:
                        ...
                        if client:
                            ...
                            if success:
                                with open(self.authed_cert, "r") as f:
                                    ca = f.read()
                                    bundle = f'{base64.b64encode(public_key.encode()).decode()}|{base64.b64encode(ca.encode()).decode()}'
                                    conn.send(bundle.encode())
                                    msg = conn.recv(1024).decode()
                                    if not msg == 'SAVED':
                                        self.clear_subnet_key(subnet, client)
                                        return (False, ())
                                    return (True, (client_id, public_key))
                                return (False, ())
                            else:
                                return (False, ())
                        else:
    ...
    def start_authed_tunnel(self, authed_socket: socket.socket):
        logger.debug(f'loading {self.authed_cert}')
        logger.debug(f'loading {self.authed_key}')
        context = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
    ...
    def get_client_id_from_subnet_cert(self, client_cert: bytes):
        try:
            db = get_db()
            public_key = x509.load_der_x509_certificate(client_cert).public_bytes(serialization.Encoding.PEM)
            encoded = base64.b64encode(public_key).decode()
            subnet = db.execute_single(
                """SELECT client_id from network_keys WHERE public_key = %s""", (encoded,)
            )
            if subnet:
                return subnet['client_id']
            return None
        except Exception as e:
            logger.error(f"Failed to get client ID associated with subnet key: {e}")
            return None
            ...
            if pending_scans:
                ...
                for scan in pending_scans:
                    if scan['subnet_netmask'] == "255.255.255.255":
                        ...
                        netmask = scan['subnet_netmask']
                        ip = scan['subnet_ip']
                        subnet = ipaddress.ip_network(f'{ip}/{netmask}',)
                        logger.debug(f"Calculated subnet for scan {scan['id']}: {subnet.compressed} with netmask {subnet.netmask}")
                ...
                db.execute_command(
                    """UPDATE scan_jobs SET status = 'running', started_at = CURRENT_TIMESTAMP WHERE id IN %s""",
                    (tuple(scan_list.keys()),)
                )
            else:
    ...
    def start_handler(self):
        ...
        auth_tunnel.start()
        authed_tunnel = Thread(target=self.start_authed_tunnel, args=(authed_socket,))
        authed_tunnel.start()
