        ...
        try:
            ...
            if not client:
                ...
                with db.get_cursor() as cursor:
                    ...
                    cursor.execute(
                        """INSERT INTO users (email, password_hash, client_admin, phone_number, verification_token, token_expires_at)
                           VALUES (%s, crypt(%s, gen_salt('bf')), TRUE, %s, %s, %s) RETURNING user_id""",
                ...
                with db.get_cursor() as cursor:
                    cursor.execute(
                        """INSERT INTO users (email, password_hash, client_admin, phone_number, verification_token, token_expires_at)
                           VALUES (%s, crypt(%s, gen_salt('bf')), FALSE, %s, %s, %s) RETURNING user_id""",
                        (email, password, formatted_phone, verification_token, token_expires_at)
                    )
                    user_id = cursor.fetchone()['user_id']
    ...
    try:
        ...
        try:
            ...
            user_data = db.execute_single(
                """SELECT u.user_id, cu.client_id, email, phone_number, client_admin, email_verified
                   FROM users u
