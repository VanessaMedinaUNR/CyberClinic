...
CONTACT_EMAIL = os.environ.get('CONTACT_EMAIL', 'contact@cyberclinic.unr.edu')
...
def send_help_request_email(user_email, client_name, issue_category, message, additional_info=None):
    html = f"""
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:auto">
      <h2 style="color:#2c3e50">New Help Request from {user_email}</h2>
      <p><strong>Client:</strong> {client_name}</p>
      <p><strong>Issue Category:</strong> {issue_category}</p>
      <p><strong>Message:</strong><br>{message}</p>
      {f'<p><strong>Additional Information:</strong> {additional_info}</p>' if additional_info else ''}
    </div>
    """
    _send(CONTACT_EMAIL, f"CyberClinic Help Request: {client_name}", html)
